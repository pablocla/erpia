...
import { informarComprobantesCaeaPendientes } from "@/lib/afip/caea-informar"
import { z } from "zod"
...
const informarSchema = z.object({
  puntoVenta: z.number().int().min(1).max(9999).optional(),
  caea: z.string().min(10).optional(),
})
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const resultado = await informarComprobantesCaeaPendientes(
      ctx.auth.empresaId,
      parsed.data.puntoVenta,
    )

    if (resultado.informados === 0 && resultado.errores.length === 0) {
      return NextResponse.json({
        ok: true,
        mensaje: "No hay comprobantes CAEA pendientes de informar",
        informados: 0,
      })
    }
    ...
    return NextResponse.json({
      ok: resultado.ok,
      informados: resultado.informados,
      errores: resultado.errores,
      mensaje: resultado.ok
        ? `${resultado.informados} comprobante(s) informados correctamente a AFIP.`
        : `${resultado.informados} informados, ${resultado.errores.length} con error.`,
    })
  } catch (error: any) {
