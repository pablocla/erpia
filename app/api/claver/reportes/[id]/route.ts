  ...
  try {
    ...
    const ticket = await db.ticket.findFirst({
      ...
      include: {
        ...
        empresa: {
          select: { id: true, nombre: true, razonSocial: true, rubro: true, email: true },
        },
      },
    })
    ...
    console.error("Error al obtener reporte CLAVER:", error)
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const existing = await db.ticket.findFirst({ where: { id: ticketId } })
    ...
    console.error("Error al actualizar reporte CLAVER:", error)
    return NextResponse.json({ error: "Error interno" }, { status: 500 })
  }
}